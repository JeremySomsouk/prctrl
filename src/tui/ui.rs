...
impl Ui {
    ...
    fn draw_pr_list(frame: &mut Frame, app: &App, area: Rect) {
        ...
        let rows: Vec<Row> = filtered_reviews
            ...
            .map(|pr| {
                ...
                Row::new(vec![
                    ...
                    Cell::from(Span::styled(format!("{}{}", pr.pr_title, draft), title_style)),
                    Cell::from(Span::styled(pr.pr_author.as_str(), Style::default().fg(Color::Cyan))),
                    Cell::from(Span::styled(pr.repo.as_str(), Style::default().fg(Color::Magenta))),
        ...
        let selected_index = app.filtered_reviews()
            .iter()
            .position(|&pr| {
    ...
    fn draw_statistics(frame: &mut Frame, app: &App, area: Rect) {
        ...
        let authors: std::collections::HashSet<_> = reviews.iter()
            .map(|pr| &pr.pr_author)
            .collect();
        let repos: std::collections::HashSet<_> = reviews.iter()
            .map(|pr| &pr.repo)
            .collect();
    ...
    fn draw_help_overlay(frame: &mut Frame, _app: &App, size: Rect) {
        let help_text = Text::from(vec![
            Line::from(vec![
                Span::styled("PRCtrl TUI - Keyboard Shortcuts", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![]),
            Line::from(vec![
    ...
    fn draw_loading_indicator(frame: &mut Frame, app: &App, size: Rect) {
        ...
        let loading_text = vec![
            ...
            Line::from(Span::styled("Please wait while we fetch your reviews", Color::Gray)),
        ];
